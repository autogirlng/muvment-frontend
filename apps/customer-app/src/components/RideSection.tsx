...
export default function RideSection() {
  return (
    <div className="w-full bg-grey-50 px-2 sm:px-4 md:px-8 py-8">
      {/* Icon and Header Row */}
      <div className="flex flex-col lg:flex-col items-center lg:items-start mb-6 max-w-6xl mx-auto px-0 w-full gap-2 lg:gap-4">
        <div className="w-10 h-10 bg-green-900 rounded-xl flex items-center justify-center mb-2 lg:mb-4 lg:ml-1">
          <Leaf className="w-8 h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 rotate-180" stroke="#4ade80" />
        </div>
        <div className="flex-1 w-full">
          ...
          <p className="text-xs md:text-sm text-grey-500 text-center lg:text-left">
            Enjoy the flexibility of renting our electric cars for as little as 1 hour or up to a full day.
          </p>
        </div>
      </div>
      {/* Main Content Card */}
      <div className="bg-white rounded-3xl p-4 sm:p-6 md:p-10 max-w-6xl mx-auto shadow-lg w-full ">
        <div className="flex flex-col md:flex-row items-center md:items-start gap-8 md:gap-60 w-full">
          ...
          <div className="flex-1 flex flex-col gap-4 max-w-full md:max-w-sm w-full">
            <div className="bg-primary-900 text-white p-4 rounded-full flex items-center gap-4 text-sm">
              <span className="w-10 h-10 flex items-center justify-center rounded-full bg-primary-500 mr-2">{Icons.ic_folder_library}</span>
              <span>Wide Selection Of Vehicles</span>
            </div>
            <div className="bg-primary-900 text-white p-4 rounded-full flex items-center gap-4 text-sm">
              <span className="w-10 h-10 flex items-center justify-center rounded-full bg-primary-500 mr-2">{Icons.ic_renewable_energy}</span>
              <span>Affordable Pricing</span>
            </div>
            <div className="bg-primary-900 text-white p-4 rounded-full flex items-center gap-4 text-sm">
              <span className="w-10 h-10 flex items-center justify-center rounded-full bg-primary-500 mr-2">{Icons.ic_bend_tool}</span>
