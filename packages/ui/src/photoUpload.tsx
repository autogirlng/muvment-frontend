import { ChangeEvent, ReactNode, useEffect, useState } from "react";
import Icons from "@repo/ui/icons";

type Props = {
  label: string;
  name: string;
  id: string;
  disabled?: boolean;
  image: ReactNode;
  onChange: (name: string, value: File | null) => void;
  value: string | File | null;
};

// file size limit

export default function PhotoUpload({
  label,
  name,
  id,
  disabled,
  onChange,
  image,
  value,
  ...rest
}: Props) {
  const [previewUrl, setPreviewUrl] = useState<string>("");

  const handleFileInputChange = (event: ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files ? event.target.files[0] : null;

    if (file) {
      onChange(name, file);
      setPreviewUrl(URL.createObjectURL(file));
    } else {
      onChange(name, null);
      setPreviewUrl("");
    }
  };

  const handleDelete = () => {
    onChange(name, null);
    setPreviewUrl("");
  };

  // Clean up the object URL when component unmounts or file changes
  useEffect(() => {
    return () => {
      if (previewUrl) {
        URL.revokeObjectURL(previewUrl);
      }
    };
  }, [previewUrl]);

  return (
    <div className="bg-grey-50 border-2 border-dashed border-grey-300 rounded-[59px]">
      <label className="h-[270px] sm:h-[400px] 3xl:h-[600px] w-full flex flex-col gap-6 items-center justify-center cursor-pointer">
        {previewUrl ? (
          <div className="h-full w-full relative">
            <img
              src={previewUrl}
              alt="Uploaded preview"
              className="h-full w-full object-cover rounded-[59px]"
            />
            <div className="absolute left-6 bottom-4">
              <button
                onClick={handleDelete}
                className="w-20 h-20 rounded-full flex justify-center items-center bg-error-50 text-error-500 *:w-[30px] *:h-[30px]"
              >
                {Icons.ic_delete}
              </button>
            </div>
          </div>
        ) : (
          <>
            {image}
            <p className="text-grey-600 text-base md:text-xl 3xl:text-h5 !font-medium">
              {label}
            </p>
            <p className="text-primary-500 text-sm md:text-base 3xl:text-h6 flex items-center gap-1">
              {Icons.ic_add_circle} <span>Add Image</span>
            </p>
            <input
              type="file"
              id={id}
              name={name}
              accept=".png, .jpg, .jpeg"
              onChange={handleFileInputChange}
              style={{ display: "none" }}
              disabled={disabled}
              {...rest}
            />
          </>
        )}
      </label>
    </div>
  );
}
